...
def computeDelta(wt, X, Xi):
	...
	n = len(X)
	M = n - 1
	meanX = sum(list(X)[:-1]) / M
	numX = 0
	for i in xrange(1, n, 1):
		numX = numX + (X['priceDiff'+str(i)] - meanX)**2
	stdX = np.sqrt(numX / M)
	num = 0
	den = 0
	for i in xrange(0, len(Xi), 1):
		Xi_row = Xi.iloc[i]
		yi = Xi_row['Yi']
		meanXi = sum(list(Xi_row)[:-1]) / M
		numXi = 0
		sim_num = 0
		for i in xrange(1, n, 1):
			numXi = numXi + (Xi_row['priceDiff'+str(i)] - meanXi)**2
			sim_num = sim_num + ((X['priceDiff'+str(i)] - meanX) * (Xi_row['priceDiff'+str(i)] - meanXi))
		stdXi = np.sqrt(numXi / M)
		similarity = sim_num / (M * stdX * stdXi)
		exp_num = math.exp(wt * similarity)
		num = num + (yi * exp_num)
		den = den + exp_num
	delta = num / den
	return delta
